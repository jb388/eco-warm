...
read_meta <- function (expName, ...) {
  ...
  if (length(list.files(exp.dir)) == 0) {
    ...
    if (sum(length(flmd.ls), length(dd.ls)) != length(mta.dir.ls)) {
      ix <- which(!(basename(dat.dir.ls) %in% c(basename(flmd.ls), basename(dd.ls))))
      message(paste(expName, "meta directory contains the following non-standard files that will be ignored:\n", basename(mta.dir.ls)[ix], "\n"))
    }
    ...
    if (length(ix) > 0) {
      message(paste("The following", expName, "data dictionary files are missing input data and will not be ingested:\n", basename(dd.ls)[ix], "\n"))
      dd.ls <- dd.ls[-ix]
    }
    ...
    if (length(ix) > 0) {
      message(paste("The following", expName, "input data files are missing data dictionaries and will not be ingested:\n", basename(dat.dir.ls)[ix], "\n"))
    }
    ...
    if (length(ix) > 0) {
      message(paste("The following", expName, "input data files are missing flmd and will not be ingested:\n", basename(dat.ls)[ix], "\n"))
      dat.ls <- dat.ls[-ix]
    }
    ...
    if (length(unlist(flmd.na)) > 0) {
      ...
      message(paste0("The following ", expName, " input files are listed in the flmd file '", names(flmd.na.nms), ".csv', but cannot be found:\n", flmd.na.nms, "\n"))
    }
...
# template for flmd
flmd_helper <- function(expName, dataFileName, dateColName, rename = FALSE, append = TRUE, write_out = TRUE, orders, ...) {
  ...
  if (Jeff) {
    ...
    nm[["emailAddress"]] <- "jbeemmiller@lbl.gov"
  }
  nm[["expName"]] <- expName
...
date.nms <- c(rep("Date", 3), rep("Date.Time..GMT..0400", 5), rep("TIMESTAMP", 5))
...
flmd_helper(
  expName = "TRACE", 
  dataFileName = "Bisley_MET_2016", 
  dateColName = "Date",
  append = TRUE, write_out = TRUE,
  Jeff = TRUE,
  orders = "%d-%b-%y")

lapply(seq_along(xlsx.TRACE.metPth2[2:14]), function(i) {
  nm <- sapply(strsplit(basename(xlsx.TRACE.metPth2[i]), ".csv"), "[[", 1)
  dateNms <- date.nms[i]
  flmd_helper(
    expName = "TRACE", 
