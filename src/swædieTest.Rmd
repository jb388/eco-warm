...
## summarize fx
difSum.ts.fx <- function(dat_in, id_col_nms = "plt_rep", dat_col_nm) {
  
  dpt_col_nms <- names(dat_in)[grep("depth", names(dat_in))]
  
  # function for calculating differences
  ctl.dif.fx <- function(df) {
      id.n <- sum(length(id_col_nms), length(dpt_col_nms), 1)
      ix.t <- seq(id.n + 1, ncol(df))
      dif.mat <- combn(ix.t, 2)
      dif <- df[dif.mat[1,]] - df[dif.mat[2,]]
      names(dif) <- paste0(names(df)[dif.mat[1,]], " - ", names(df)[dif.mat[2,]])
      cbind(df, dif)
  }
    ...
    id.n <- sum(length(id_col_nms), length(grep("depth", names(dat))), 1, length(unique(dat_in$plt_treat)))
    ix.t <- seq(id.n + 1, ncol(dat)) 
    
    sum <- dat %>%
      mutate(dateDay = date(date)) %>%
      group_by(dateDay, pick(all_of(dpt_col_nms))) %>%
      summarize(
        across(.cols = names(dat)[ix.t], .fns = list(mean = ~ mean(.x, na.rm = T), sd = ~ sd(.x, na.rm = T))), .groups = "drop") %>%
      pivot_longer(
     ...
     plt_sum <- dat %>%
       ...
       pivot_longer(
        ...
        mutate(dif = str_remove(dif, "_mean$"),
               dif = str_remove(dif, "_sd$"))
     list(subDaily = dat, daily = sum, dailyPlt = plt_sum)
}

plt.fx <- function(expName, sum.ls, dat_bks, dat_lbs, DIF = "all", show_plots = TRUE, subtitle = NULL, facet_col = "dif", units = "pct") {
  
  dayMean <- sum.ls$daily %>%
    filter(stat == "mean") %>%
    mutate(col = ifelse(data < 0, "#b5651d", "#1db565"))
  
  pltMean <- sum.ls$dailyPlt %>%
    filter(stat == "mean") %>%
    mutate(col = ifelse(data < 0, "#b5651d", "#1db565"))
  
  if (DIF != "all") {
    dayMean <- dayMean %>% slice(grep(DIF, dayMean$dif))
    pltMean <- pltMean %>% slice(grep(DIF, pltMean$dif))
  }
  
  if (length(grep("depth", names(dayMean))) == 2) {
    dayMean <- dayMean %>% mutate(depth = mean(c(depth_upper, depth_lower)))
    pltMean <- pltMean %>% mutate(depth = mean(c(depth_upper, depth_lower)))
  }
  
  if (units != "pct") {
    dayMean <- dayMean %>%
      filter(dayMean$data < 1) %>%
      mutate(data = data * 100)
    pltMean <- pltMean %>%
      filter(pltMean$data < 1) %>%
      mutate(data = data * 100)
  } else {
    dayMean <- dayMean %>%
      filter(dayMean$data < 100)
    pltMean <- pltMean %>%
      filter(pltMean$data < 100)
  }
  ...
  if (show_plots) {
    p <- pltMean %>%
      ...
      geom_point(
        data = dayMean,
        aes(dateDay, data, color = col), size = .1) +
      scale_color_manual(
        name = "",
    ...
    p <- dayMean %>%
      filter(!is.na(data)) %>%
      ggplot(., aes(dateDay, data, color = col)) +
...
KAEF.sm.sum.ls <- difSum.ts.fx(
  ...
  id_col_nms = c("plt_block", "plt_rep"),
  dat_col_nm = "vwc_pct")
plt.fx("KAEFS", KAEF.sm.sum.ls, "1 year", "%Y", DIF = "control - treatment3")
plt.fx("KAEFS", KAEF.sm.sum.ls, "1 year", "%Y", DIF = "control - treatment3", show_plots = FALSE)


# SPRUCE
SPRC.sm.ls <- filPltTbl.fx(moist.ls, "SPRUCE")
SPRC.sm.sum.ls <- lapply(SWTR.sm.ls, function(x) {
  difSum.ts.fx(x, dat_col_nm = "H2O")
})
SPRC.sm.sum.ls2 <- do.call(Map, c(rbind, SPRC.sm.sum.ls))
plt.fx("SPRUCE", SPRC.sm.sum.ls2, "1 year", "%Y", DIF = "control - treatment")
...
SWTR.sm.sum.ls <- difSum.ts.fx(
  SWTR.sm.ls[[1]],
  dat_col_nm = "H2O")
plt.fx("SWELTR", SWTR.sm.sum.ls, "4 months", "%b-%y")
plt.fx("SWELTR", SWTR.sm.sum.ls, "1 year", "%Y", show_plots = FALSE)
...
TEAM.sm.sum.ls <- difSum.ts.fx(
  TEAM.sm.ls[[1]],
  dat_col_nm = "moist")
plt.fx("TEAM", TEAM.sm.sum.ls, "1 year", "%Y")
plt.fx("TEAM", TEAM.sm.sum.ls, "1 year", "%Y", show_plots = FALSE)
...
plt.fx("TeRaCON", TRCN.sm.sum.ls, "1 year", "%Y", show_plots = FALSE)
...
```{r moist-ts-prep}
p.save <- function(expName, p) {
  dir <- "../docs/AGU24"
  fnm <- file.path(dir, paste0(expName, "_sm.png"))
  if (file.exists(fnm)) {
    fnm <- file.path(dir, paste0(expName, "_sm1.png"))
  }
  ggsave(
    filename = fnm,
    plot = p,
...
p.save(expName = "CiPEHR", plt.fx("CiPEHR", CPHR.sm.sum.ls, "1 year", "%y", DIF = "control - treatment", facet_col = "plt_subBlock"))

p.save(expName = "CiPEHR", plt.fx("CiPEHR", CPHR.sm.sum.ls, "1 year", "%y", DIF = "control - treatment", show_plots = FALSE))

p.save(expName = "Harvard Forest", plt.fx("Harvard Forest", HRVF.sm.sum.ls, "1 year", "%Y", DIF = "control - treatment", units = "ppn"))

p.save(expName = "Harvard Forest", plt.fx("Harvard Forest", HRVF.sm.sum.ls, "1 year", "%Y", DIF = "control - treatment", show_plots = FALSE, units = "ppn"))

p.save(expName = "KAEFS", plt.fx("KAEFS", KAEF.sm.sum.ls, "1 year", "%Y", DIF = "control - treatment"))

p.save(expName = "KAEFS", plt.fx("KAEFS", KAEF.sm.sum.ls, "1 year", "%Y", DIF = "control - treatment", show_plots = FALSE))

p.save(expName = "SPRUCE", plt.fx("SPRUCE", SPRC.sm.sum.ls2, "1 year", "%Y", DIF = "control - treatment", units = "ppn"))

p.save(expName = "SWELTR", plt.fx("SWELTR", SWTR.sm.sum.ls, "4 months", "%b-%y", units = "ppn"))

p.save(expName = "SWELTR", plt.fx("SWELTR", SWTR.sm.sum.ls, "1 year", "%Y", show_plots = FALSE, units = "ppn"))

p.save(expName = "TEAM", plt.fx("TEAM", TEAM.sm.sum.ls, "1 year", "%Y", units = "ppn"))

p.save(expName = "TEAM", plt.fx("TEAM", TEAM.sm.sum.ls, "1 year", "%Y", show_plots = FALSE, units = "ppn"))

p.save(expName = "TeRaCON", plt.fx("TeRaCON", TRCN.sm.sum.ls, "1 year", "%Y"))
       
p.save(expName = "TeRaCON", plt.fx("TeRaCON", TRCN.sm.sum.ls, "1 year", "%Y", show_plots = FALSE))
```
...
dat.rng <- lapply(seq_along(moist.clean.ls), function(i) {
  x <- moist.clean.ls[[i]]
  if (any(grepl("daily", names(x)))) {
    df <- data.frame(max = max(x$daily$data, na.rm = T),
               min = min(x$daily$data, na.rm = T))
  } else {
    df <- bind_rows(lapply(x, function(y)
      data.frame(max = max(y$daily$data, na.rm = T),
                 min = min(y$daily$data, na.rm = T)) 
      )) %>%
