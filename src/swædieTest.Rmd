...
# checks column names
checkColNms <- function(tableName, datIn, err, verbose = TRUE, ...) {
  
  vcat(paste("\t", tableName, "\n"))
  ...
  if (!any(grepl(tableName, names(datIn)))) {
    err <- err + 1
    vcat(warning(
      paste("\t", tableName, "is missing in metadata/siteData directory\n")))
  }
  ...
  for (i in seq_along(req)) {
    if (!any(grepl(req[i], names(datIn[[tableName]])))) {
      err <- err + 1
      vcat(warning(
        paste("\t\t", "Required column", req[i], "is missing in", tableName, "table\n")))
    }
  }
...
coreData.fx <- function(DIR = "../data/sweddie", write_report = TRUE, write_out = TRUE, verbose = TRUE) {
  ...
  for (d in seq_along(database)) {
    ...
    for (i in seq_along(datIn)) {
      ...
      # check column names
      err <- checkColNms(names(datIn)[i], datIn, err)
  ...
  # add plot index to plot table
  database <- lapply(database, function(x) {
    if (!("plt_ind" %in% names(x$plot))) {
      if (nrow(x$plot) > 0) {
        plt_ind <- 1:nrow(x$plot)
        x$plot <- cbind(x$plot, plt_ind)  
      }
    }
    return(x)
  })
...
getFLMD_DD <- function(exp_names = NULL) {
  ...
  flmd_dd.ls <- lapply(
    lapply(exp_names, read_meta), function(x) {
      if (length(x$flmd) == 0) {
        NULL
      } else {
        x
...
chkNms <- function(expName) {
  lapply(moist.ls[[expName]], function(x) unique(x$plt_name) %in% database[[expName]]$plot$plt_name)
}
...
# remove data from plots 7 / ambient and select relevant columns
moist.ls$Blodgett[[1]] <- moist.ls$Blodgett[[1]][-which(moist.ls$Blodgett[[1]][["PP"]] == "ambient" | moist.ls$Blodgett[[1]][["PP"]] == "7"), c("TIMESTAMP", "PP", "moisture", "depth", "trt")]
...
# "X"         "TIMESTAMP" "PP"        "variable"  "moisture"  "date"      "depth"    "trt"

lapply(moist.ls, function(x) lapply(x, names))
...
moist.ls$CiPEHR[[1]] <- moist.ls$CiPEHR[[1]] %>%
  select(block, fence, plot, ts, vwc)

# block, fence, plot, ts, vwc)
...
# "date"   "p1_sm"  "p2_sm"  "p3_sm"  "p4_sm"  "p5_sm"  "p6_sm"  "p7_sm"  "p8_sm" "p9_sm"  "p10_sm" "p11_sm" "p12_sm" "p13_sm" "p14_sm" "p15_sm" "p16_sm" "p17_sm" "p18_sm"


# KAEFS
moist.ls$KAEFS <- list(read.csv("/Users/jeff/eco-warm/data/experiments/KAEFS/input_data/SoilMoisture2018-2023_reshapeLong.csv"))

# "Timestamp" "plotID"    "depth"     "vwc_pct" 


# SPRUCE
moist.ls$SPRUCE <- lapply(list.files("/Users/jeff/eco-warm/data/experiments/SPRUCE/input_data", full.names = TRUE)[1:7], read.csv)
names(moist.ls$SPRUCE) <- paste0("soil_", seq(2016, 2022))

# Note: VWC missing from "soil_2016"; VERT


# SWELTR
moist.ls$SWELTR <- list(read.csv("/Users/jeff/eco-warm/data/experiments/SWELTR/input_data/SWELTR_to2020_T_H2O.csv"))

# "TIME"  "DEPTH" "T"     "H2O"   "PLOT"  "TREAT"


# TEAM
moist.ls$TEAM <- list(read.csv("/Users/jeff/eco-warm/data/experiments/TEAM/input_data/moist.csv"))

# "depth" "time"  "plot"  "moist"


# TeRaCON
moist.ls$TeRaCON <- list(read.csv("/Users/jeff/eco-warm/data/experiments/TeRaCON/raw/Biocon_hourly_warmingseason_2012-2018vprimary1.csv"))



# TRACE
moist.ls$TRACE <- list(
  surface = read.csv("/Users/jeff/eco-warm/data/experiments/TRACE/meta/TRACE_SurfaceSoil_2015-2018_dd.csv"),
  deep = read.csv("/Users/jeff/eco-warm/data/experiments/TRACE/meta/TRACE_DeepSoil_2016-2018_dd.csv"))
