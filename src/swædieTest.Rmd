...
# heat levels
database2 <- Filter(Negate(is.null), database)
hlvls <- as.numeric(unique(unlist(lapply(lapply(database2, function(x) separate_longer_delim(x$experiment, "heat_levels", ";")), "[[", "heat_levels"))))
plot_hlvls <- data.frame(htlvls = hlvls[hlvls != 0 & !is.na(hlvls)])
p <- ggplot(plot_hlvls, aes(htlvls)) +
...
ggsave(
    filename = "/Users/jeff/eco-warm/docs/AGU24/hlvls.png",
    plot = p,
    height = 12.49, 
    width = 21.48, 
    dpi = 300,
...
test<-bind_rows(lapply(database, function(x) lapply(x$experiment, as.character)))
# parse dates
BG_i <- parse_date_time(test$heat_BG_date_i, orders = c("%Y", "%m/%d/%y", "%m/%d/%Y", "%b-%y", "%d-%b"))
AG_i <- parse_date_time(test$heat_AG_date_i, orders = c("%Y", "%m/%d/%y", "%m/%d/%Y", "%b-%y", "%d-%b"))
dur.BG <- time_length(interval(BG_i, today()), "year")
dur.AG <- time_length(interval(AG_i, today()), "year")

test$belowground <- dur.BG
test$aboveground <- dur.AG
...
ggsave(
    filename = "/Users/jeff/eco-warm/docs/AGU24/dur.png",
    plot = p,
    height = 12.49, 
    width = 21.48, 
    dpi = 300,
...
moist.ls$B4WarmED <- list(
  read.csv("/Users/jeff/eco-warm/data/experiments/B4WarmED/raw/B4WarmED_2009-2023_SoilMoisture.csv")
)
# filter to: non NA, closed canopy, water = ambient
moist.ls$B4WarmED[[1]] <- moist.ls$B4WarmED[[1]] %>%
  filter(!is.na(soil_moisture__cm.3H2Oxcm..3soil)) %>%
  filter(water_treatment == "ambient") %>%
  filter(canopy == "closed") %>%
  mutate(site = ifelse(site == "cfc", "Cloquet", "Ely")) %>%
  rename(sit_name = site, vwc = soil_moisture__cm.3H2Oxcm..3soil)
moist.ls$B4WarmED[[1]]$timeStamp_AmericaRegina <- substr(moist.ls$B4WarmED[[1]]$timeStamp_AmericaRegina, 1, nchar(moist.ls$B4WarmED[[1]]$timeStamp_AmericaRegina) - 1)
...
# SPRUCE
moist.ls$SPRUCE <- lapply(list.files("/Users/jeff/eco-warm/data/experiments/SPRUCE/input_data", pattern = "l.csv", full.names = TRUE)[2:8], read.csv)
...
filPltTbl.fx <- function(ls, expName, heat_only = TRUE, orders = NULL, plt_cols = NULL, dateName = "date", ...) {
  ...
  lapply(seq_along(ls[[expName]]), function(i) {
    ...
    # parse date/time
    x$date <- parse_date_time(x[[dateName]], orders = orders[[i]])
...
# SPRUCE
SPRC.sm.ls <- lapply(filPltTbl.fx(moist.ls, "SPRUCE", orders = rep("ymd HM", length(moist.ls$SPRUCE)))
...
# TeRaCON
TRCN.sm.ls <- filPltTbl.fx(moist.ls, "TeRaCON", orders = "mdy HM")
...
ggplot(moist.clean.m.ls$ACBB, aes(month, `treatment4 - control_mean`, color = sit_name)) +
  geom_hline(yintercept = 0) +
  geom_point() +
  geom_errorbar(
    aes(ymin = `treatment4 - control_ci.05`, ymax = `treatment4 - control_ci.95`),
    width = .2) +
...
eco.df <- data.frame(
  ...
  eco = c("cropland", "cropland", "grassland", "temperate forest", "permafrost", "temperate forest", "grassland", "boreal forest", "tropical forest", "grassland", "grassland")
) %>%
...
moist.clean.ls$SPRUCE$subDaily$depth <- as.numeric(ifelse(moist.clean.ls$SPRUCE$subDaily$depth == 1, 4, 20))
...
p <- sm.mean.df %>%
  ...
  geom_point(
    ...
    position = position_dodge(width = 0.5),
    stroke = 2) +
  scale_size_continuous(name = "", range = c(1, 9), breaks = c(2, 4, 6, 8)) +
  scale_shape_manual(
    name = "",
   ...
   rowwise() %>%
      mutate(
        ...
        across(
          ...
          .names = "{sub('_mean$', '', .col)}_ci.95")) %>%
      ungroup() %>%
      select(sit_name, month, depth_cat, contains("_mean"), contains("_ci.")) %>%
      pivot_longer(
        cols = contains("- control"),
        names_to = c("dif", ".value"),
        names_pattern = "^(.*)_(.*)$")
...
sm.mean.m.df$eco <- eco.df[match(sm.mean.m.df$sit_name, eco.df$sit_name), "eco"]
sm.mean.m.df$wrm <- wrm.df[match(sm.mean.m.df$sit_name, wrm.df$sit_name), "wrm"]
sm.mean.m.df$biome <- eco.df[match(sm.mean.m.df$sit_name, eco.df$sit_name), "biome"]
...
ggsave(
    filename = "/Users/jeff/eco-warm/docs/AGU24/month_meanSMdif.png",
    plot = p,
    height = 23.62, 
    width = 37.94, 
    dpi = 300,
...
sm.mean.m.df3 %>%
  ...
  mutate(`temp. dif.` = as.numeric(str_extract(dif, "\\d+\\.*\\d*")),
         ...
         ai = ai * .0001) %>%
  rename(`site name` = sit_name) %>%
  ggplot(., aes(month, mean, color = ai, group = group)) +
  geom_hline(yintercept = 0) +
  geom_line() + 
  geom_point(
    aes(x = month + jitter_offset, mean, shape = biome, size = `temp. dif.`)) +
...
ggsave(
    filename = "/Users/jeff/eco-warm/docs/AGU24/eco.png",
    plot = p,
    height = 12.49, 
    width = 21.48, 
    dpi = 300,
...
# 185Exp
temp.ls$`185ExperimentStation` <- list(read.csv( "/Users/jeff/eco-warm/data/experiments/185ExperimentStation/input_data/datafile_soilW.csv"))


# ACBB
temp.ls$ACBB <- list(
  cropland = read.csv("/Users/jeff/eco-warm/data/experiments/ACBB/input_data/temp_C.csv"),
  grassland = read.csv("/Users/jeff/eco-warm/data/experiments/ACBB/input_data/temp_G.csv"))
temp.ls$ACBB <- setNames(lapply(seq_along(temp.ls$ACBB), function(i) {
  x <- temp.ls$ACBB[[i]]
  x$plt_name <- paste0(
    names(temp.ls$ACBB)[i], "_", ifelse(x$treat == "T", 4, 0), "_", x$plot)
  x
...
p.save(expName = "ACBB", plt.fx("ACBB", ACBB.st.sum.ls2, "3 months", "%b", facet_col = "sit_name", VAR = "deg C", datType = "soil temperature"))
...
ACBB.st.sm.df %>%
  ...
  ggplot(., aes(data_st, data_sm, color = plt_treat)) + 
  geom_smooth(method = "lm", se = F) +
  geom_point(aes(shape = as.character(plt_rep)), alpha = .01) +
  facet_grid(cols = vars(sit_name), rows = vars(depth), scales = "free") +
  theme_bw() +
  theme(panel.grid = element_blank())

test <- lapply(split(ACBB.st.sm.df, ACBB.st.sm.df$sit_name), function(x) {
    lmer(data_sm ~ data_st + plt_treat * depth + (1 | plt_rep), x)
})
test2 <- lapply(split(ACBB.st.sm.df, ACBB.st.sm.df$sit_name), function(x) 
  lmer(data_sm ~ data_st * plt_treat * depth + as.factor(month) + (1 | plt_rep), x))
test3 <- 
  lmer(data_sm ~ data_st + plt_treat * depth + sit_name + (sit_name|plt_rep), ACBB.st.sm.df)

# look for the mean differences by month at each site, then look at the relationship between temperature and soil moisture * depth, with site as random factor


# B4W
st.B4WarmED.in <- list.files("/Users/jeff/eco-warm/data/experiments/B4WarmED/input_data", full.names = TRUE)
moist.ls$B4WarmED <- lapply(st.B4WarmED.in, read.csv)

# Blodgett
temp.ls$Blodgett <- list(read.csv("/Users/jeff/eco-warm/data/experiments/Blodgett/input_data/20240125_processed Blodgett soil temperature.csv"))
...
BLDG.st.ls <- filPltTbl.fx(temp.ls, "Blodgett", orders = "%y-%m-%d %H:%M:%S", dateName = "TIMESTAMP")
ACBB.st.sum.ls <- setNames(lapply(ACBB.st.ls, function(x) {
  difSum.ts.fx(x, dat_col_nm = "data")
}), nm = names(temp.ls$ACBB))
...
```{r texture}
ACBB.sand <- read.csv("/Users/jeff/eco-warm/data/experiments/ACBB/input_data/Sand.csv")
